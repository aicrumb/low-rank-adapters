...
def add_lora_to_gpt_j(model, adapter_rank):
    for i in range(len(model.transformer.h)):
        lora_layer = LoRALinear(model.transformer.h[i].attn.k_proj.in_features, model.transformer.h[i].attn.k_proj.out_features, adapter_rank)
        ...
        lora_layer = LoRALinear(model.transformer.h[i].attn.v_proj.in_features, model.transformer.h[i].attn.v_proj.out_features, adapter_rank)
        ...
        lora_layer = LoRALinear(model.transformer.h[i].attn.q_proj.in_features, model.transformer.h[i].attn.q_proj.out_features, adapter_rank)
